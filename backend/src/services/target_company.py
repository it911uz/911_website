...
class TargetCompanyManager:
    def __init__(self, db: AsyncSession):
        self.repo = TargetCompanyRepository(db, TargetCompany)
        self.click_repo = ClickRepository(db, Click)
    ...
    async def create_target(
        ...
        return TargetCompanyResponse(
            ...
            url=f"https://it911.uz/c/{target_company.id}",
    ...
    async def update_target(
        ...
        target_company = await self.repo.get(target_company_id)
    ...
    async def get_target(
        ...
        lead_filter = LeadFilter(target_id=target_company.id)
        leads = await self.lead_repo.list(lead_filter)
        return TargetCompanyResponse(
            id=target_company.id,
            name=target_company.name,
            is_active=target_company.is_active,
            ...
            clicks=len(clicks),
            leads=len(leads.get("data")),
        )

    @redis_cache(prefix="target:list", ttl=120)
        ...
        return TargetCompanyListResponse(
            data=[
                TargetCompanyResponse(
                    ...
                    leads=len(
                        (
                            await self.lead_repo.list(
                                LeadFilter(target_id=target_company.id)
                            )
                        ).get("data")
                    )
                )
                for target_company in target_companies.get("data")
            ],
