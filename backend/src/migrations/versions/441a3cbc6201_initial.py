...
Revision ID: 441a3cbc6201
Revises: 
Create Date: 2025-10-28 11:45:16.696701
...
def upgrade() -> None:
    """Upgrade schema."""
    # Удаляем возможные старые типы ENUM перед созданием
    op.execute("DROP TYPE IF EXISTS taskstatusenum CASCADE;")
    op.execute("DROP TYPE IF EXISTS statusenum CASCADE;")
    op.execute("DROP TYPE IF EXISTS dealstatusenum CASCADE;")
    ...
    sa.Column('status', sa.Enum('NEW', 'PROCESSING', 'CANCELLED', name='taskstatusenum', create_type=False), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.Column('deleted_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    ...
    sa.Column('status', sa.Enum('NEW', 'PROCESSING', 'CANCELLED', 'DEAL', name='statusenum', create_type=False), nullable=True),
    ...
    sa.Column('status', sa.Enum('SUCCESS', 'PROCESSING', 'CANCELLED', name='dealstatusenum', create_type=False), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.Column('deleted_at', sa.DateTime(), nullable=True),
