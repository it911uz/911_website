...
from core.schemas import TimeStampSchema, validate_hex_color
...
class LeadMinRead(LeadBase, TimeStampSchema):
    id: int
...
class LeadFileMinRead(BaseModel):
    id: uuid.UUID
    filename: str

class LeadFileRead(LeadFileMinRead):
    url: str


class LeadRead(LeadMinRead):
    id: int
    status_id: Optional[int]

    comments: list[LeadCommentRead] = []
    status: Optional[LeadStatusRead] = None
    files: list[LeadFileMinRead] = []
    model_config = {
        "from_attributes": True
    }
